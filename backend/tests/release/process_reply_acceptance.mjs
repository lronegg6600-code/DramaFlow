...
import { appendLog, buildVerifiedBody, canUseGithub, execGh, loadTicketMap, saveTicketMap, writeNamedEvidence } from "./_issue_ops_helpers.mjs";
...
const verifiedIds = new Set(
  validations.flatMap((payload) =>
    (payload.items || []).filter((item) => item.current_status === "received_and_verified").map((item) => item.blocker_id)
  )
);

for (const item of ticketMap.items.filter((entry) => verifiedIds.has(entry.blocker_id))) {
  if (item.issue_lifecycle_status !== "verified" && item.issue_lifecycle_status !== "closed") {
    const body = buildVerifiedBody(item);
    if (mode === "real" && item.issue_number) {
      execGh(["issue", "comment", String(item.issue_number), "--repo", process.env.GITHUB_REPOSITORY, "--body", body]);
      appendLog("blocker-comment-log.json", { blocker_id: item.blocker_id, issue_number: item.issue_number, comment_type: "accepted", created_at: generatedAt, body });
    }
    ...
    item.issue_lifecycle_status = "verified";
    acceptanceItems.push({ blocker_id: item.blocker_id, issue_number: item.issue_number, mode });
  }
}

saveTicketMap(ticketMap);
writeNamedEvidence("blocker-acceptance-log.json", { generatedAt, mode, items: acceptanceItems });
appendTransitionLog(transitionItems, generatedAt);

console.log("[process_reply_acceptance] pass");

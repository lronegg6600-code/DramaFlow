...
for (const item of ticketMap.items.filter((entry) => entry.issue_number && entry.issue_lifecycle_status === "replied_invalid")) {
  const reason = "input package still invalid after latest validation";
  const body = buildReopenedBody(item, reason);
  if (mode === "real") {
  ...
  items.push({ blocker_id: item.blocker_id, issue_number: item.issue_number, mode, reason });
}

saveTicketMap(ticketMap);
writeNamedEvidence("blocker-reopen-log.json", { generatedAt, mode, items });

console.log("[comment_on_blocker_reopened] pass");

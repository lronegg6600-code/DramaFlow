...
func TestLoginReturnsValidationEnvelopeOnBadJSON(t *testing.T) {
	gin.SetMode(gin.TestMode)

	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/v1/admin/auth/login", strings.NewReader("{"))
	ctx.Request.Header.Set("Content-Type", "application/json")

	handler := New(service.Service{}, sharedtelemetry.NewMetrics("admin-service-test"))
	handler.login(ctx)

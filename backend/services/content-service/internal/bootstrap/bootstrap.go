...
import (
	...
	"dramaflow/backend/services/content-service/internal/app"
	"dramaflow/backend/services/content-service/internal/repository"
...
func Build(ctx context.Context) (*http.Server, func(context.Context) error, error) {
	cfg, err := sharedconfig.Load("content-service", "8082")
	if err != nil {
		return nil, nil, err
	}

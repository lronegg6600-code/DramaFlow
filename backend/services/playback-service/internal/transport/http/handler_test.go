...
func TestCreateSessionReturnsValidationEnvelopeOnBadJSON(t *testing.T) {
	gin.SetMode(gin.TestMode)

	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/v1/playback/sessions", strings.NewReader("{"))
	ctx.Request.Header.Set("Content-Type", "application/json")

	handler := New(service.Service{}, sharedtelemetry.NewMetrics("playback-service-test"), gin.TestMode)

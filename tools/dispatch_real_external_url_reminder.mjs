...
const payload = {
  ...
  items: [],
  reason: result.ok ? "Reminder for resolvable external staging URLs dispatched to platform issue." : (result.reason || result.stderr || "Failed to dispatch reminder."),
};
await writeJson(path.join(EVIDENCE_DIR, "staging-external-url-second-reminder-log.json"), payload);
